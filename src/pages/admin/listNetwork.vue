                            ...
                            <div>
                                <q-markup-table>
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-for="(item, index) in networkList" :key="item.id">
    ...
    methods: {
        ...
        async addNetwork() {
            ...
            try {
                ...
                const response = await this.$api.post(`/order/create/network`, data, {
                    headers: {
        ...
        async getNetworkData() {
            const token = sessionStorage.getItem("token")
            try {
                const response = await this.$api.get(`media/network/all`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
        ...
        async editNetwork(id) {
            const token = sessionStorage.getItem("token")
            try {
                const response = await this.$api.patch(`media/network/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
        ...
        async getNetworkById(id) {
            const token = sessionStorage.getItem("token")
            try {
                const response = await this.$api.get(`media/network/by-id/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                ...
                console.log(response);
                this.dataNetwork = response.data
                this.edit = true
